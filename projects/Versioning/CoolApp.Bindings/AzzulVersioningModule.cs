...
namespace DustInTheWind.CoolApp
{
    internal class AzzulVersioningModule : VersioningModule
    {
        public AzzulVersioningModule(Configuration config)
            : base(config)
        {
            var url = VersionCheckerConfig.Url;

            if (string.IsNullOrEmpty(url))
                VersionCheckerConfig.Url = "http://azzul.alez.ro/appinfo.xml";

            VersionChecker.AppName = "Azzul";
            VersionChecker.CurrentVersion = new Version(1, 2, 3, 4);
        }
    }
}